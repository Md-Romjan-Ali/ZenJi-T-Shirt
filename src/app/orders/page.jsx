                ...
                {orders.length ? <>
                    <div className="mt-8 overflow-x-auto rounded-xl border border-neutral-200 dark:border-neutral-800">
                        <table className="w-full min-w-[680px] text-left text-sm">
                            <thead className="bg-neutral-100 text-[11px] uppercase tracking-widest text-neutral-500 dark:bg-[#111111]"><tr><th className="px-5 py-4">Product</th><th className="px-5 py-4">Size</th><th className="px-5 py-4">Quantity</th><th className="px-5 py-4">Price</th><th className="px-5 py-4 text-right">Total</th></tr></thead>
                            <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">{orders.map(order => <tr key={order.id}><td className="px-5 py-4"><div className="flex items-center gap-4"><Image src={order.image} alt={order.productName} width={56} height={56} className="h-14 w-12 rounded object-cover" /><span className="font-semibold">{order.productName}</span></div></td><td className="px-5 py-4 font-mono">{order.size}</td><td className="px-5 py-4 font-mono">{order.quantity}</td><td className="px-5 py-4 font-mono">৳{order.price}</td><td className="px-5 py-4 text-right font-mono font-bold">৳{order.total}</td></tr>)}</tbody>
                        </table>
                    </div>
